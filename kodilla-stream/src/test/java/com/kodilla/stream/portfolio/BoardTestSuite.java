...
public class BoardTestSuite {
    ...
    @Test
    public void testAverageTimeTaskInProgress() {
        //Given
        Board project = prepareTestData();
        ...
        List<TaskList> inProgressTasks = new ArrayList<>();
        inProgressTasks.add(new TaskList("In progress"));
        // Wersja z dwoma streamami
        long counter = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .map(Task::getCreated)
                .map(date -> DAYS.between(date, LocalDate.now()))
                .count();
        ...
        double avg = LongStream.range(0, project.getTaskLists().stream()
                    .filter(inProgressTasks::contains)
                    .flatMap(tl -> tl.getTasks().stream())
                    .map(Task::getCreated)
                    .map(date -> DAYS.between(date, LocalDate.now()))
        ...
        Assert.assertEquals(10, avg, 0.0001);
    }
}